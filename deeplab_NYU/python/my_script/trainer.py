...
def train_runner(solver, train_set, model):
    #solstate = 'voc12_2cam_woback/model/vgg128_noup/train_iter_15000.solverstate'
    cmd = os.environ['CAFFE_DIR'] + os.environ['CAFFE_BIN'] + ' train' \
    ' --solver=' + os.environ['CONFIG_DIR'] + '/' + solver + '_' + train_set + '.prototxt' \
    ' --weights=' + model + ' --gpu=0' #os.environ['DEV_ID'] # ##change solver ' --weights=' + model + ' --snapshot=' + solstate
    print ('Running ' + cmd)
