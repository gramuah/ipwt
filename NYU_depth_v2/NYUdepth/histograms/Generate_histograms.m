...
load('../Code/nyu_depth_v2_labeled.mat');
load('../Code/splits.mat');
...
for i=1:size(ScenesTrain)
    images{i,1} = ['../Images/', ScenesTrain{i,1}(1:end-4), '.png'];
end
for i=1:size(ScenesTest)
    images{i+size(ScenesTrain,1),1} = ['../Images/', ScenesTest{i,1}(1:end-4), '.png'];
end
for i=1:size(ScenesTrain)
    imagesSS{i,1} = ['../Annotations40/', ScenesTrain{i,1}(1:end-4), '.png'];
end
for i=1:size(ScenesTest)
